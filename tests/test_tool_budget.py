...
class TestToolPriority:
    ...
    def test_has_eight_entries(self) -> None:
        """_TOOL_PRIORITY should have exactly 8 entries (7 core + verify_plan)."""
        assert len(server_mod._TOOL_PRIORITY) == 8
...
class TestToolBudgetFiltering:
    ...
    async def test_no_env_var_keeps_all(self) -> None:
        """Without MIRDAN_TOOL_BUDGET, all 8 tools should remain."""
        env = os.environ.copy()
