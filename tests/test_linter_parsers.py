...
class TestRuffParser:
    ...
    def test_parse_security_rule_severity(self) -> None:
        raw = json.dumps([
            {"code": "S105", "message": "Hardcoded password", "location": {"row": 1, "column": 1}},
        ])
        violations = parse_ruff_output(raw)
...
class TestMypyParser:
    ...
    def test_parse_multiline_output(self) -> None:
        lines = [
            json.dumps({"file": "a.py", "line": 1, "column": 1, "severity": "error", "message": "err1", "code": "E"}),
            json.dumps({"file": "a.py", "line": 2, "column": 1, "severity": "error", "message": "err2", "code": "E"}),
        ]
        raw = "\n".join(lines)
        violations = parse_mypy_output(raw)
        assert len(violations) == 2
...
class TestEslintParser:
        ...
        raw = json.dumps([
            {
                "filePath": "/src/app.ts",
                "messages": [
                    {"ruleId": "no-undef", "severity": 2, "message": "undef", "line": 1, "column": 1}
                ],
            }
        ])
        violations = parse_eslint_output(raw)
    ...
    def test_parse_multiple_files(self) -> None:
        raw = json.dumps([
            {
                "filePath": "/src/a.ts",
                "messages": [{"ruleId": "r1", "severity": 1, "message": "m1", "line": 1, "column": 1}],
            },
            {
                "filePath": "/src/b.ts",
                "messages": [{"ruleId": "r2", "severity": 2, "message": "m2", "line": 2, "column": 2}],
            },
        ])
        violations = parse_eslint_output(raw)
        assert len(violations) == 2
        ...
        raw = json.dumps([
            {
                "filePath": "/src/bad.ts",
                "messages": [
                    {"ruleId": None, "severity": 2, "message": "Parsing error", "line": 1, "column": 1}
                ],
            }
        ])
        violations = parse_eslint_output(raw)
