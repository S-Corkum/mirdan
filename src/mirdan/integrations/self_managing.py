...
class SelfManagingIntegration:
    ...
    def generate_quality_context(
        ...
        lines = [
            ...
            "",
            "mirdan is active in this project. `enhance_prompt` is optional and recommended",
            "for security-sensitive, multi-file, or new-library work. `validate_code_quality`",
            "after writing code remains mandatory — fix all errors before marking complete.",
            "",
        ]
...
### After Writing Code
Call `mcp__mirdan__validate_code_quality(code)` to validate:
- Pass `check_security=true` for security-sensitive files
- Fix ALL errors before the task is complete
